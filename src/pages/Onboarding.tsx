...
export function Onboarding() {
  ...
  // Adding an ADDITIONAL shop (reached from ShopSwitcher's "Add Another Shop")
  // is not supported yet: the backend keys each shop to a single YeboID owner
  // (Shop.ownerYeboidSub is unique) and has no createShop endpoint. Rather than
  // walk the user through a form that can't persist — which previously faked a
  // success and dropped them on a phantom shop that vanished on reload — we tell
  // them plainly. (This screen is also why createShop() now fails loudly.)
  if (isNewShop) {
    return (
