...
export function TillBanner() {
  ...
  return (
    <Link
      to="/cash-up"
      className={`flex items-center justify-between gap-3 mb-3 px-4 py-2.5 rounded-sharp border transition-colors ${
        isOpen
          ? 'bg-ok/10 border-ok/30 hover:bg-ok/15'
          : 'bg-wash border-ink/30 hover:bg-brand/15'
      }`}
    >
      <div className="flex items-center gap-2.5 min-w-0">
        {isOpen ? (
          <>
            ...
            <BanknotesIcon className="w-5 h-5 text-ok shrink-0" />
            <span className="text-sm text-ok truncate">
          ...
          <>
            <LockOpenIcon className="w-5 h-5 text-brick shrink-0" />
            <span className="text-sm text-brick truncate">No till open — open one to track cash</span>
          </>
        )}
      </div>
      <span className="flex items-center gap-1 text-xs font-semibold shrink-0 text-ink/80">
